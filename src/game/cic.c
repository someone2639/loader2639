...
static u32 si_crc32(const u8 *data, size_t size);

// Determines the CIC seed for a cart, given the ROM data.
//int get_cic_seed(const u8 *rom_data, u32 *cic_seed) {
int get_cic(unsigned char *rom_data) {
    u32 crc = si_crc32(rom_data + 0x40, 0x1000 - 0x40);
    u32 aleck64crc = si_crc32(rom_data + 0x40, 0xC00 - 0x40);
...
u32 si_crc32(const u8 *data, size_t size) {
  u32 table[256];
  unsigned n, k;
  u32 c;
